...
/* ==========================================================
 🏀 重構重整 + 修錯版
 - 修正多處語法錯誤（缺分號、殘留變數名、函式截斷等）
 - 移除殘留的 debug 呼叫，保留 ?reset=1 / ?preset=1 行為
 - 維持：columns 驅動 DataTable、單一 calcStats、統一排序器 cmp
 - 不改既有測試；另外補幾個小測試
========================================================== */
...
const colorDiverge = (v,min,max,pos="#09734E",neg="#7D2D2D")=> colorScale({v,min,max,mode:'div',pos,neg});
...
// ==========================================================
// 欄位工廠：Player/Team 共用 columns 生成器
// ==========================================================
function makePlayerColumns({ onPlayerClick, includeTeam=false, posMode='split', includeSalaries=false }={}){
  const cols = [
    { key:'Rank', label:'Rank', sort:'num' },
    { key:'PLAYER', label:'PLAYER', sort:'str', render:(p)=> (
        <span className="text-blue-400 underline-offset-2 hover:underline cursor-pointer" onClick={()=> onPlayerClick && onPlayerClick(p)}>{p.PLAYER}</span>
      ) },
  ];
  if(includeTeam){ cols.push({ key:'TEAM', label:'TEAM', sort:'str' }); }
  if(posMode==='merge'){
    cols.push({ key:'POS', label:'POS', sort:'str', compute:(p)=> (p.POS + (p.POS2?` / ${p.POS2}`:'')) });
  }else{
    cols.push({ key:'POS', label:'POS', sort:'str' });
    cols.push({ key:"POS'", src:'POS2', label:"POS'", sort:'str' });
  }
  cols.push(
...
// ==========================================================
function PlayerTab({app,setApp,goPlayerCard,stats,isAdmin}){
  const players = app?.players || [];
  const [sortKey,setSortKey] = useState('Rank');
  const [sortAsc,setSortAsc] = useState(true);

  const columns = useMemo(() =>
    makePlayerColumns({ onPlayerClick: goPlayerCard, includeTeam: true, posMode: 'split', includeSalaries: true }),
  [goPlayerCard]);
  ...
  return (
    <div className="max-w-[2400px] mx-auto px-6 py-6">
      <Section title="球員表（點欄位可排序）" right={isAdmin ? (
...
function PlayerCard({app,setApp,player,back,allPlayers,selectPlayer,goTeam,isAdmin}){
  ...
  async function onUploadCard(file){ const url = await readFileAsDataURL(file); const nextPlayers = (app.players||[]).map(x=> x.id===player.id ? {...x, cardImage:url} : x); const next = {...app, players: nextPlayers}; setApp(next); const updated = nextPlayers.find(x=>x.id===player.id); if(updated) selectPlayer(updated); }
  return (
    <div className="max-w-[1200px] mx-auto px-6 py-6 space-y-4">
...
function TeamDetail({abbr, app, setApp, openPlayerCard, onSwitchTeam, isAdmin, stats}){
  ...
  const [sortAsc,setSortAsc] = useState(true);
  const columns = useMemo(() =>
    makePlayerColumns({ onPlayerClick: openPlayerCard, includeTeam: false, posMode: 'merge', includeSalaries: false }),
  [openPlayerCard]);
  ...
  const salaryColumns = useMemo(() =>
    makePlayerColumns({ onPlayerClick: openPlayerCard, includeTeam: false, posMode: 'merge', includeSalaries: true }),
  [openPlayerCard]);

  async function onUploadDepth(file){ const url=await readFileAsDataURL(file); const next={...app, teamImages:{...app.teamImages,[abbr]:url}}; setApp(next); }
...
export function runTests(){ try{
  ...
  assert('cmp numeric asc', cmp(1,2,true) < 0); assert('cmp numeric desc', cmp(1,2,false) > 0);
...
export default function App(){
  async function tryLoadPresetOnce(current){ try{ if(typeof window==='undefined') return; if(FORCE_PRESET){ try{ localStorage.removeItem(STORAGE_KEY); localStorage.removeItem(PRESET_FLAG_KEY);}catch(e){} } const already=localStorage.getItem(PRESET_FLAG_KEY);
    const empty=!current || !Array.isArray(current.players) || current.players.length===0;
    // 若資料為空或強制載入，就抓 preset
    if(!FORCE_PRESET){
      const haveLoaded=!!already;
      const hasData=!empty;
      if(haveLoaded && hasData){ return; } // 只有「已載入且目前有資料」才跳過
    }
    const url=getPresetURL(); const res=await fetch(url,{cache:'no-store'}); if(!res.ok){ console.warn('Preset fetch not ok:', res.status); return; }
    const data=await res.json(); const merged={...DEFAULT_STATE, ...data}; setApp(merged); localStorage.setItem(PRESET_FLAG_KEY,'1'); console.log('Preset loaded from', url, '(forced:', FORCE_PRESET, ')'); }catch(err){ console.warn('Preset load skipped:', err); } }
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      ...
      {isAdmin && (
        <div className="w-full px-6 py-4 flex items-center gap-2">
          <FileLabel accept="application/json" onChange={e=>e.target.files?.[0]&& (async (f)=>{ try{ const text=await readFileAsText(f); const data=JSON.parse(text); const merged={...DEFAULT_STATE,...data}; setApp(merged);} catch(err){ alert('JSON 匯入失敗：'+(err?.message||err)); } })(e.target.files[0])}>匯入 JSON</FileLabel>
      ...
      {tab==='Player' && !playerCard && (
        <PlayerTab app={app} setApp={setApp} goPlayerCard={openPlayerCard} stats={stats} isAdmin={isAdmin} />
      )}
      {tab==='Player' && playerCard && (
